...
def app():
    ...
    dane["weekday"]=dane["Timestamp"].dt.strftime('%w')
    dane["Month"]=dane["Timestamp"].dt.strftime('%A')

    month_average_course= dane.groupby(['Month','weekday']).mean(numeric_only=True).unstack()
